...
void imageCallback(const sensor_msgs::ImageConstPtr& msg_){
  ...
  cv::Mat tempImg;
  cv::cvtColor(cvimg->image, tempImg, COLOR_BGR2HSV);
  ...
  cv::cvtColor(tempImg, cameraFeed, COLOR_HSV2BGR);
  ...
  while(true){
    ...
    if (cv::waitKey(30) == 27){ //if 'esc' is pressed then terminate program
      exit(EXIT_FAILURE);
    }

    if (cv::waitKey(30) == 107){ //while 'k' is pressed then continue the program
      x = 0;
      y = 0;
      break;
    }
